...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, channel, joint_num, time_len, domain):
        ...
        if channel % 2 != 0:
            raise ValueError("Channel must be an even number. Got {}".format(channel))
        ...
        if div_term.shape[0] != channel // 2:
            raise ValueError("Mismatch in div_term size. Expected {}, got {}".format(channel // 2, div_term.shape[0]))
    ...
    def forward(self, src_input):
        ...
        if self.input_type == 'keypoint':
            ...
            body_head = self.body_visual_head(
                x=body,
                mask=src_input['mask'].cpu(),
                valid_len_in=src_input['new_src_lengths'].cpu())
        ...
        for k in ['left', 'right', 'fuse', 'body']:
            outputs[f'recognition_loss_{k}'] = self.compute_recognition_loss(
                gloss_labels=src_input['gloss_input']['gloss_labels'].cpu(),
                gloss_lengths=src_input['gloss_input']['gls_lengths'].cpu(),
                gloss_probabilities_log=head_outputs[f'{k}_gloss_probabilities_log'],
                input_lengths=src_input['new_src_lengths'].cpu())
